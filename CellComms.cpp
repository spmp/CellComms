...
/**
 * Vector Accumulator function (x, y) -> z
 **/
...
uint8_t CellComms::balancingState(void) {
	uint8_t balanceNum = balancingNum();
	if (balanceNum == 0) return BMS_BALANCING_NONE;
	if (balanceNum == cellNum) return BMS_BALANCING_ALL;
 ...
 * Cell numbers
 **/
 uint8_t  CellComms::millivoltsMaxCell(void) {
 	// Get a vector of cell millivolts
 	std::vector<uint16_t> tempVector = millivoltsVect();

 	// return the maximum cell number
 	return std::distance(
 		tempVector.begin(),
 		std::max_element(tempVector.begin(), tempVector.end())
 	) + 1 ;
 }

uint8_t  CellComms::millivoltsMinCell(void) {
	// Get a vector of cell millivolts
	std::vector<uint16_t> tempVector = millivoltsVect();

	// return the minimum cell number
	return std::distance(
		tempVector.begin(),
		std::min_element(tempVector.begin(), tempVector.end())
	) + 1 ;
}

uint8_t  CellComms::temperatureMaxCell(void) {
 // Get a vector of cell millivolts
 std::vector<uint16_t> tempVector = temperatureVect();
...
uint8_t  CellComms::temperatureMinCell(void) {
 // Get a vector of cell millivolts
 std::vector<uint16_t> tempVector = temperatureVect();
